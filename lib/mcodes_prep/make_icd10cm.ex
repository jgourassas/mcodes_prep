defmodule McodesPrep.MakeIcd10cm do
  ...
  def xml_file do
    tabular_file_string =
      IO.gets("\t Location of Tabular File i.e. : data/icd10cm_tabular_2022.xml > ")
      |> String.trim()
  ...
  def make_cm_orders do
    ...
    case response do
      "Y" ->
        ...
        order_file_string =
          IO.gets("\t Location of order File i.e. : data/icd10cm_order_2022.txt > ")
          |> String.trim()
  ...
  def make_alternative() do
    ...
    case response do
      "Y" ->
        ...
        colorize_text(
          "default",
          "Please  from inside psql mcodes_prep_repo run  the commands/shape_icd10cm.sql"
        )
  ...
  def search_icd10cm do
    ...
    query =
      Ecto.Query.from(p in McodesPrep.Icd10cm,
        order_by: [asc: p.icd10cm_code_2],
        where:
          fragment("(?) @@ plainto_tsquery(?)", p.long_description, ^"#{term}%") or
            fragment("(?) ilike(?)", p.icd10cm_code_2, ^"#{term}%"),
        # select: [ p.icd10cm_code_2, p.long_description],
        select: %{
          codealso: p.codealso,
  ...
  def show_record() do
    ...
    for rec <- record do
      ...
      if chapter_name !== "" do
        colorize_text(
          "default",
          IO.ANSI.italic() <> "Chapter Name:  " <> IO.ANSI.not_italic() <> "#{chapter_name}"
        )
      end
      ...
      if chapter_desc !== "" do
        colorize_text(
          "default",
          IO.ANSI.italic() <> "Chapter Description: " <> IO.ANSI.not_italic() <> "#{chapter_desc}"
        )
      end
      ...
      if section_id !== "" do
        colorize_text(
          "default",
          IO.ANSI.italic() <> "Section: " <> IO.ANSI.not_italic() <> "#{section_id}"
        )
      end
      ...
      if section_description !== "" do
        colorize_text(
          "default",
          IO.ANSI.italic() <>
            "Section Description: " <> IO.ANSI.not_italic() <> "#{section_description}"
        )
      end
      ...
      if is_category !== nil do
        colorize_text(
          "default",
          IO.ANSI.italic() <> "Is Category:  " <> IO.ANSI.not_italic() <> "#{is_category}"
        )
      end
      ...
      if is_subcategory !== nil do
        colorize_text(
          "default",
          IO.ANSI.italic() <> "Is  Sub Category:  " <> IO.ANSI.not_italic() <> "#{is_subcategory}"
        )
      end
      ...
      if is_header !== nil do
        colorize_text(
          "default",
          IO.ANSI.italic() <> "Is Header:  " <> IO.ANSI.not_italic() <> "#{is_header}"
        )
      end
  ...
  def response_options() do
    response =
      get_response(" Search in ICD-10-CM Again? [Y,N] / Show Single Record [R] / View Abbr[A]  ")
  ...
  def view_abbraviations() do
    ...
    colorize_text(
      "default_write",
      "ca = Code Also, cf = Code First, ex1 = excludes1, ex2 = excludes2, inc = Includes, incT = inclusion Term, uaC = Use Additional Code"
    )
