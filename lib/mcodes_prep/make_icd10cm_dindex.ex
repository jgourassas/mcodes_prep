defmodule McodesPrep.MakeIcd10cmDindex do
  ...
  def set_dindex_xml_file do
    IO.gets("\t Location of ICD10-CM DRUG INDEX XML file i.e: data/icd10cm_drug_2022.xml  > ")
    |> String.trim()
  end
  ...
  defp insert_dindex(map) do
    ...
    Enum.each(main_terms_l, fn main_term ->
      ...
      # {:ok, inserted} =
      McodesPrep.Repo.insert(%McodesPrep.Icd10cmDindex{title: main_title, main_term: terms_map})
    end)
  ...
  defp show_tabular(term) do
    ...
    if main_cell_l != nil do
      main_cell_row =
        Enum.reduce(main_cell_l, [], fn x, acc ->
    ...
    if terms_l != nil do
      Enum.each(terms_l, fn term ->
        ...
        for a <- term_a do
          ...
          ##############
          term_cell_row =
            Enum.reduce(term_cell_l, [], fn x, acc ->
              [x[:cell_code] | acc]
            end)
    ...
    |> Table.put_column_meta(1..5, align: :left)
    |> Table.render!()
    |> IO.puts()
