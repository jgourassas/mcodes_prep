defmodule McodesPrep.MakeIcd10cmIndex do
  ...
  def set_cm_index_xml_file do
    IO.gets("\t Location of ICD10-PCS INDEX XML file i.e: data/icd10cm_index_2018.xml  > ")  |> String.trim
  end
  ########################
  ...
  defp insert_icd10cm_index(map) do
    ...
    Enum.map(main_term_l, fn(l) ->
      ...
      McodesPrep.Repo.insert(%McodesPrep.Icd10cmIndex{title: title,
                                                        main_term_jsonb: l})
    end )
  ...
  def search_cm_index do
    ...
    response = get_response( "Search Again in  ICD-10-CM Alphabetic Index ? [Y,N] / Search in Tabular[T]") |> String.upcase
    case response do
      "Y" ->
        search_cm_index()
      "T" ->
