defmodule McodesPrep.MakeIcd10pcs do
  ...
  def make_pcs_orders do
    ...
    case response do
      "Y" ->
        pcs_order_string =
          IO.gets("\t Location of ICD10-PCS order file i.e:  data/icd10pcs_order_2022.txt > ")
          |> String.trim()
  ...
  def set_pcs_xml_file do
    ...
    IO.gets("\t Location of ICD10-PCS XML file i.e: data/icd10pcs_tables_2022.xml > ")
    |> String.trim()
  end
  ...
  def get_pcs_from_tabular(list) do
    ...
    icd10pcs_code_1 =
      a_section[:section] <> a_body_system[:body_system] <> a_root_op[:root_operation]
  ...
  def make_all_pcs_set_axis_fields() do
    colorize_text(
      "default",
      "Seting  ICD-10-PCS axis Fields Values i.e section 0, body_system W, .. Estimated Time:  1198005.697  ms or  19 min"
    )
      ...
      Ecto.Query.from(p in McodesPrep.Icd10pcs,
        ...
        where:
          fragment("(?) @@ plainto_tsquery(?)", p.long_description, ^"#{term}%") or
            fragment("(?) ilike(?)", p.icd10pcs_code, ^"#{term}%"),
  ...
  def response_options() do
    response =
      get_response("Search in ICD-10-PCS Again? [Y,N] / Show Single Record [R]")
      |> String.upcase()
