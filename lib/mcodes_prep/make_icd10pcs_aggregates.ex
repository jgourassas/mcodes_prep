defmodule McodesPrep.MakeIcd10pcsAggregates do
 ...
 def insert_icd10pcs_agg(map) do
   ...
   all_body_sys =  Enum.reduce(body_sys_l, " ", fn(x, acc) ->
               %{body_sys: v} = x
                acc <> "^" <> v
      end)# Enum body_sys_l
   ...
   McodesPrep.Repo.insert(%McodesPrep.Icd10pcsAggegates{device: device,
                                                   operation: operation,
    ...
    end) # Enum each aggregate_l
    colorize_text("alert",  "---Finish -------------------------")
  end # insert_icd10pcs_agg
...
def search_pcs_aggregates do
    ...
    device_q =
      ...
      select: %{device: fragment("? ", p.device),
    ...
    response = get_response( "Search Again in ICD-10-PCS Aggregates  File? [Y,N]") |> String.upcase
