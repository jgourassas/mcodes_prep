defmodule McodesPrep.MakeIcd10pcsDefs do
  ...
  def insert_icd10pcs_defs(map) do
    ...
    Enum.each(axis_l, fn(l) ->
      ...
      Enum.each(terms_l, fn(term_l) ->
        ...
        term_titles = Enum.reduce(term_titles_l, " ", fn (x, acc) ->
          %{term_title: v} = x
          acc <> "^" <> v
        end)

        term_includes_l = term_l[:term_includes_l]
        term_includes =  Enum.reduce(term_includes_l, " ",  fn(x, acc)->
          %{term_include: v} = x
          acc <> "^" <>  v
        end)
        ...
        # {:ok, inserted} =
        McodesPrep.Repo.insert(%McodesPrep.Icd10pcsDefs{section: section,
  ...
  def search_pcs_defs do
    ...
    main_term_q =
      ...
      select: %{terms: fragment("? ", p.terms), section_title: fragment(" ? ",  p.section_title),
    ...
    response = get_response( "Search Again in ICD-10-PCS Definitions File? [Y,N]") |> String.upcase
