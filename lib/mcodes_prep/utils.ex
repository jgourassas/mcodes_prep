defmodule McodesPrep.Utils do
  ...
  def slice_line_orders(line) do
    order_number = String.slice(line, 0..5)
    |> String.trim_trailing

    icd10cm_code = String.slice(line, 6..13)
    |> String.trim_trailing

    is_header = String.slice(line, 14..15)
    |> String.trim_trailing

    short_description = String.slice(line, 16..76)
    |> String.trim_trailing
  ...
  def slice_line_orders1(line) do
    ...
    icd10_code = String.slice(line, 6..13)
    |> String.trim_trailing
  ...
  def present_tablerex(records, header, title) do
    ...
    |> Table.put_column_meta(2, color: fn(text, value) -> if value in ["1"], do: [:green, text], else: [:red, text] end)
    |> Table.put_column_meta(0, color: :blue) # sets column header to red, too
    |> Table.put_column_meta(1..2, align: :left) # `1..2` is a range of column indexes. :all also works.
  ...
  def print_searching_in(title) do
    IO.puts " "
    IO.puts IO.ANSI.black_background() <> IO.ANSI.white()
    <> " \u25A3 \u0391\u03A9 ---------- \u2766  #{title} \u2766 ---------- \u25A3 " <> IO.ANSI.reset
    IO.puts " "
  end
...
##################
  def print_results_title(title) do
    IO.puts " "
    IO.puts IO.ANSI.magenta_background() <> IO.ANSI.white()
    <> " \u25A3 \u0391\u03A9 ---------- \u25C9  Results For: #{title} \u25C9 ---------- \u25A3 " <> IO.ANSI.reset
    IO.puts " "
